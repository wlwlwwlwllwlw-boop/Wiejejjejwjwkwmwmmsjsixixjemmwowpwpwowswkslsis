...
-- Menggunakan sistem UI dari raw (1).txt dan logika fitur dari deepseek_lua
-- Semua fitur palsu, loading screen, dan placeholder telah dihapus.
...
-- // 1. Load Library UI dari pastefy
local success, Library = pcall(function()
    return loadstring(game:HttpGetAsync("https://pastefy.app/YoX4PJmf/raw"))()
end)

if not success or not Library then
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "DARK HUB";
        Text = "Gagal memuat UI Library. Periksa koneksi atau link pastefy.";
        Duration = 5;
    })
    return
end
...
local Window = Library:CreateWindow({
    ...
    Logo = "rbxassetid://94549241134796",
    LogoSize = 32,
    SphereText = false,
    SphereWords = "DH",
    SphereImage = "rbxassetid://94549241134796",
    SphereIconSize = 38
})
...
local SocialsTab = Window:CreateTab("Socials", false, false)
local SettingsTab = Window:CreateTab("Settings", false, false)

local M_Page1 = MainTab:CreatePage("Player")
local M_Page2 = MainTab:CreatePage("Auto Farm")
local C_Page1 = CombatTab:CreatePage("Combat")
local V_Page1 = VisualsTab:CreatePage("Visuals")
local S_Page1 = SettingsTab:CreatePage("Settings")
local Soc_Page1 = SocialsTab:CreatePage("Links")
...
local function autoFarmLoop()
    while autoFarmEnabled do
        if not getCharacter() then task.wait(1) goto continue end
        ...
        if not autoFarmEnabled then break end
        local positions = { Vector3.new(-467, 693, 41), Vector3.new(-475, 693, 45) }
...
local function canSeeTarget(targetHead)
    ...
    if result then return result.Instance:IsDescendantOf(targetHead.Parent) end
    return true
end

-- // 9. Main Tab - Page 1 (Player)
local PlayerCard = M_Page1:CreateSection("Player Modules")
PlayerCard:AddToggle("Infinite Stamina", false, function(state) _G.InfiniteStaminaEnabled = state end, {
    Title = "Infinite Stamina",
    Description = "Locks stamina at 100 so you can sprint endlessly."
})
PlayerCard:AddToggle("Auto Pickup", false, function(state)
    _G.AutoLoot = state
    if state then
        task.spawn(function()
...
PlayerCard:AddToggle("Instant Interact", false, function(state) _G.InstantInteractEnabled = state end, {
    Title = "Instant Interact",
    Description = "Removes hold timers on proximity prompts."
})
PlayerCard:AddToggle("No Jump Cooldown", false, function(state) _G.NoJumpCooldownEnabled = state end, {
    Title = "No Jump Cooldown",
    Description = "Removes jump cooldown for continuous jumping."
})
PlayerCard:AddToggle("Noclip", false, function(state) _G.NoclipEnabled = state end, {
    Title = "Noclip",
    Description = "Allows you to walk through walls."
})
PlayerCard:AddToggle("Safe Speed Boost", false, function(state) _G.WalkSpeedEnabled = state end, {
    Title = "Safe Speed Boost",
    Description = "Boost your speed slightly to avoid anti-cheat detection."
})

-- // 10. Main Tab - Page 2 (Auto Farm)
local FarmCard = M_Page2:CreateSection("Auto Farm Controls")
FarmCard:AddToggle("Spawn Auto Farm UI", false, function(state)
...
FarmCard:AddToggle("Freeze Auto Farm UI", false, function(state)
...
-- // 11. Combat Tab - Page 1
local CombatCard = C_Page1:CreateSection("Combat Modules")
CombatCard:AddToggle("Spawn Aimbot UI", false, function(state)
...
CombatCard:AddSlider("Aimbot FOV", 40, 300, 120, function(val)
    aimbotFOV = val
...
CombatCard:AddToggle("Freeze Aimbot UI", false, function(state)
...
-- // 12. Visuals Tab - Page 1
local VisualsCard = V_Page1:CreateSection("Visual Modules")
VisualsCard:AddToggle("Max Zoom Out", false, function(state)
...
VisualsCard:AddToggle("Player ESP", false, function(state) _G.ESPEnabled = state end, {
    Title = "Player ESP",
...
VisualsCard:AddDropdown("Select Player", playerList, false, function(selected)
...
VisualsCard:AddButton("Inspect Inventory", function()
    ...
    if char then
        ...
        if backpack then
            for _, tool in ipairs(backpack:GetChildren()) do
                if tool:IsA("Tool") then table.insert(inventory, tool.Name .. " (Equipped)") end
            end
        end
    end
...
-- // 13. Socials Tab
local SocCard = Soc_Page1:CreateSection("Socials")
SocCard:AddButton("Copy Discord Link", function()
    if setclipboard then setclipboard("https://discord.gg/xKvegCV6yf") end
    Library:Notify({Title = "Copied", Description = "Discord link copied!", Duration = 3})
end, {
    Title = "Copy Discord Link",
    Description = "Copies the invite link to your clipboard."
})
SocCard:AddButton("Copy YouTube Link", function()
    if setclipboard then setclipboard("https://youtube.com/@strixwashere") end
    Library:Notify({Title = "Copied", Description = "YouTube link copied!", Duration = 3})
end, {
    Title = "Copy YouTube Link",
    Description = "Copies the YouTube channel link to your clipboard."
})

-- // 14. Settings Tab - Page 1
local SettingsCard = S_Page1:CreateSection("General Settings")
SettingsCard:AddButton("FPS Booster (Potato Graphics)", function()
    ...
    Library:Notify({Title = "FPS Booster", Description = "Graphics stripped for performance.", Duration = 4})
end, {
    Title = "FPS Booster (Potato Graphics)",
    Description = "Strips heavy textures and effects to boost FPS."
})
SettingsCard:AddConfigManager("DARKHUB_Config")
SettingsCard:AddButton("Unload Script", function()
    pcall(function()
        autoFarmGui:Destroy()
        aimbotGui:Destroy()
        ...
        _G.ESPEnabled = false
        autoFarmEnabled = false
        for player, _ in pairs(espData) do removeESP(player) end
...
toggleBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then dragMobile = false end
        end)
    end
end)
...
RunService.Heartbeat:Connect(function()
    if _G.InfiniteZoomEnabled then LocalPlayer.CameraMaxZoomDistance = 100000 end
