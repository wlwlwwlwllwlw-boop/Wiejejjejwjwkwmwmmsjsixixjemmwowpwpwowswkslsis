-- ================================================================
-- LENGER STORE — CARD SCAM ONLY v3 (BUG FIXED)
...
-- ================================================================
-- STEP 3 — WAIT APPROVAL (FIXED REGEX)
-- ================================================================
local function checkApprovalNotif()
    ...
    -- ⚠️ CEK "NOT SUCCESS" DULU — karena mengandung kata "success"
    ...
    then
        return false, notif.Text -- REJECTED
    end
    ...
    then
        return true, notif.Text -- APPROVED
    end

    return nil, notif.Text -- notif lain, ignore
end
...
local function step_WaitApproval(maxWait)
    ...
    while tick() - startT < maxWait do
        ...
        if result == true then
            ...
            task.wait(2) -- tunggu notif hilang biar gak stale
            return true
...
local function step_ClaimCard()
    ...
    -- Tunggu CardPickup spawn (max 10s)
    local card
    for i = 1, 20 do
        if not getgenv().CARD_SCAM_RUNNING then return false end
    ...
    if not card then
        setStatus("CardPickup belum spawn", Color3.fromRGB(255,80,80))
        return false
    end
...
local function cardScamLoop()
    ...
    while getgenv().CARD_SCAM_RUNNING do
        ...
        if approvalResult == true then
            ...
            -- Timeout tapi tidak rejected — tetap coba claim
...
print("[LENGER] Card Scam Only v3 BUG FIXED loaded")