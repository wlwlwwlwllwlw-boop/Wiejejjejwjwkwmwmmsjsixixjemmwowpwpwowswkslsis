...
-- v3: Jeda diperlambat + Panasin loop + Homeless ROTASI 1→12→1
-- ============================================================
...
local SC_HOMELESS = {
    Vector3.new(-315.35, 3.72,  -361.56),   -- 1
    Vector3.new(-273.52, 3.85,  -211.32),   -- 2
    Vector3.new(1102.42, 3.36,  527.05),    -- 3
    Vector3.new(52.89,   3.72,  -425.36),   -- 4
    Vector3.new(152.88,  3.73,  -210.08),   -- 5
    Vector3.new(-522.75, -7.86, -165.08),   -- 6
    Vector3.new(65.12,   3.73,  68.10),     -- 7
    Vector3.new(26.04,   3.73,  217.89),    -- 8
    Vector3.new(520.08,  3.87,  -295.52),   -- 9
    Vector3.new(699.28,  3.72,  -427.05),   -- 10
    Vector3.new(900.03,  3.94,  -283.12),   -- 11
    Vector3.new(874.89,  3.73,  -63.02),    -- 12
}
...
local ST = {
    ...
    cycle        = 0,
    targetChips  = 5,
    homelessIdx  = 1,   -- rotasi homeless berikutnya (1..12)
}
...
-- ============================================================
-- DELAY TABLE (DIPERLAMBAT / DIPERKETAT)
-- ============================================================
local DELAY = {
    afterTP      = 1.2,
    equipSettle  = 0.5,
...
local function buyUntilEnough(index, itemName, target, maxAttempts)
    ...
    while ST.running and attempt < maxAttempts do
        ...
        repeat task.wait(0.08) until getCount(itemName) >= target or os.clock() > deadline
        if getCount(itemName) >= target then return true end
        task.wait(0.2)
    end
...
local function waitUntilConsumed(toolName, timeout)
    ...
    while ST.running and (os.clock() - t0) < timeout do
        if getCount(toolName) < before then
            task.wait(0.4)
            return true
        end
        task.wait(0.1)
...
local function waitEquip(name, timeout)
...
local function farmOneChip()
    ...
    equipTool("Potato")
    waitEquip("Potato", 2.5)
    task.wait(DELAY.equipSettle)

    updateStatus("🥔 Kentang masuk plastik...", Color3.fromRGB(255, 200, 80))
    task.wait(DELAY.promptFire)
...
local function doFullCycle()
    while ST.running do
        ...
        -- ============ FASE 2: PANASIN (loop sampai habis) ============
        ...
        if rawCount > 0 then
            ...
            local attempts   = 0
            local maxAttempt = rawCount + 5
            local lastCount  = countPotatoChips()

            while ST.running and countPotatoChips() > 0 and attempts < maxAttempt do
                attempts = attempts + 1
                task.wait(DELAY.promptFire)
                firePromptAt(SC_TUKAR, 30)
                task.wait(DELAY.panasinWait)

                local nowCount = countPotatoChips()
                updateCycle(("Siklus #%d · Panasin (%d sisa, attempt %d)"):format(
                    ST.cycle, nowCount, attempts))

                if nowCount >= lastCount and attempts >= 3 and nowCount == lastCount then
                    break
                end
                lastCount = nowCount
            end
        ...
        -- ============ FASE 3: SELL HOMELESS (rotasi 1→12→1) ============
        local hotCount = getCount("Hot Chips")
        if hotCount > 0 then
            updateStatus(("🏠 Jual %d Hot Chips (rotasi Homeless)..."):format(hotCount),
                Color3.fromRGB(0, 220, 100))

            equipTool("Hot Chips")
            waitEquip("Hot Chips", 2.5)
            task.wait(0.6)
            if not ST.running then break end

            local safety    = 0
            local maxSafety = hotCount + #SC_HOMELESS
            ...
            while ST.running and getCount("Hot Chips") > 0 and safety < maxSafety do
                ...
                updateStatus(("🏠 Homeless #%d · Sisa Hot: %d"):format(hIdx, getCount("Hot Chips")),
                    Color3.fromRGB(0, 220, 100))
                updateCycle(("Siklus #%d · Homeless #%d/%d · Sisa Hot: %d"):format(
                    ST.cycle, hIdx, #SC_HOMELESS, getCount("Hot Chips")))

                if not LP.Character:FindFirstChild("Hot Chips") then
                    equipTool("Hot Chips")
                    waitEquip("Hot Chips", 2.0)
                    task.wait(0.3)
                end

                local beforeHot = getCount("Hot Chips")
                ...
                if ok then
                    ...
                    firePromptAt(hPos, 30)
                    task.wait(0.6)
                    firePromptAt(hPos, 30)
                    task.wait(DELAY.afterSell)

                    local afterHot = getCount("Hot Chips")
                    if afterHot < beforeHot then
                        updateStatus(("✅ Homeless #%d berhasil (terjual %d)"):format(
                            hIdx, beforeHot - afterHot),
                            Color3.fromRGB(0, 220, 100))
                    else
                        updateStatus(("⏭ Homeless #%d cooldown, lanjut..."):format(hIdx),
                            Color3.fromRGB(200, 180, 100))
                    end
                end

                -- Rotasi maju ke homeless berikutnya (meski gagal)
                ST.homelessIdx = ST.homelessIdx + 1
                if ST.homelessIdx > #SC_HOMELESS then
                    ST.homelessIdx = 1
                end
            ...
            if ST.running and getCount("Hot Chips") > 0 then
                updateStatus(("⚠ Sisa %d Hot → TP Chips Cook..."):format(getCount("Hot Chips")),
                    Color3.fromRGB(255, 160, 60))
                hopTP(SC_COOK)
                task.wait(0.8)
            end
...
infoTxt.TextSize = 10
infoTxt.Text = "🔄 Farm → Panasin → Sell → Ulangi\n🔁 Homeless rotasi #1→#12→#1 (auto lanjut)\n🚀 TP Hop 150 stud/s (tanpa respawn)"
...
print("[AUTO FULL CYCLE v3] Loaded! Homeless rotasi 1→12→1, Panasin loop, jeda diperlambat.")