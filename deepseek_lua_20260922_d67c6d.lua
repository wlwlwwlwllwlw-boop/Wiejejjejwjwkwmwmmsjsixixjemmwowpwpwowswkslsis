...
-- v2: Jeda diperlambat, tiap step tunggu selesai dulu
-- ============================================================
...
-- ============================================================
local DELAY = {
    afterTP      = 1.2,   -- settle setelah TP sebelum mulai aksi
    equipSettle  = 0.5,   -- tunggu tool benar-benar ter-equip
    promptFire   = 0.5,   -- jeda sebelum proximity di-fire
    insertPotato = 4.0,   -- setelah kentang masuk, tunggu proses selesai
    cookProcess  = 4.0,   -- setelah prompt cook, tunggu reaksi server
    insertFlour  = 4.0,   -- setelah flour masuk, tunggu proses selesai
    potOpen      = 1.5,   -- tunggu pot benar-benar kebuka
    claimChips   = 3.0,   -- tunggu chip benar-benar masuk inventory
    panasinWait  = 3.0,   -- tunggu tukar selesai
    sellWait     = 0.6,   -- tunggu prompt homeless ke-fire
    afterSell    = 0.4,   -- jeda antar homeless
    cookTime     = 60,
}
...
-- WAIT HELPERS
-- ============================================================
-- Tunggu sampai item benar-benar berkurang dari inventory.
-- Timeout default 10s (diperlambat dari 6s).
local function waitUntilConsumed(toolName, timeout)
    ...
    while ST.running and (os.clock() - t0) < timeout do
        if getCount(toolName) < before then
            -- settle tambahan supaya server sempat memproses penuh
            task.wait(0.4)
            return true
        end
        task.wait(0.1)
...
-- Tunggu sampai tool muncul di character (benar-benar ter-equip)
...
local function farmOneChip()
    ...
    -- ================================================
    -- Step B — Insert Potato (tunggu selesai dulu)
    -- ================================================
    ...
    -- tunggu kentang benar-benar habis dari inventory (max 10s)
    waitUntilConsumed("Potato", 10)
    -- tunggu proses internal server selesai (jeda panjang)
    task.wait(DELAY.insertPotato)
    if not ST.running then return false end

    -- ================================================
    -- Step C — Cook (tunggu reaksi server selesai)
    -- ================================================
    ...
    -- ================================================
    -- Step D — Insert Flour (tunggu selesai dulu)
    -- ================================================
    ...
    -- ================================================
    -- Step E — Buka Pot (tunggu pot benar-benar buka)
    -- ================================================
    ...
    -- ================================================
    -- Claim Chips (tunggu masuk inventory dulu)
    -- ================================================
...
print("[AUTO FULL CYCLE v2] Loaded! Jeda diperlambat + tiap step tunggu selesai.")