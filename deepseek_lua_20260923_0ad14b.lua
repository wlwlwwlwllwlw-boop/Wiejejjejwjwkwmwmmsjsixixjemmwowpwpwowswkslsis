...
-- State Machine · Verifikasi inventory · Vehicle TP · Retry
-- ================================================================
...
-- ================================================================
-- KONSTANTA (dari script asli — JANGAN diubah)
-- ================================================================
-- Apartment (fully.lua)
local ApartmentData = {
...
-- ================================================================
-- TP — universal hop (vehicle + character)
-- ================================================================
local tpBusy = false
...
-- Discrete step for underground blink
...
-- BypassTP — used for apartment purchase (requires respawn)
...
-- STEP: GO DEALER + GET MOTOR + WAIT SEATED
...
-- STEP: BUY MARSHMALLOW INGREDIENTS
...
local function stepAddWater()
    ...
    -- put water (no long wait — water will be ready when we return later)
...
local function stepApplyCard()
    if getFakeIDCount() == 0 then
        -- Fake ID already used or missing; skip if card exists
...
-- STEP: WAIT APPROVAL (shorter — checking notification)
local function stepWaitApproval()
    setStatus("WAITING APPROVAL...", Color3.fromRGB(255, 200, 80))
    local t0 = tick()
    while STATE.running and tick() - t0 < 90 do
        local res, _ = checkApprovalNotif()
...
-- STEP: CHIPS SEQUENCE (A→B(potato)→C→D(flour)→pot)
local function stepChipsSequence()
    ...
    -- Step E — Open pot (start cook timer)
...
-- STEP: ADD SUGAR + GELATIN (marshmallow)
...
-- STEP: ADD EMPTY BAG (collect marshmallow)
...
local function stepClaimCard()
    ...
    -- Wait for CardPickup object to exist
    local card = nil
    for _ = 1, 30 do
        if not STATE.running then return false end
        card = findCardPickup()
    ...
    -- Card object found — TP to it and claim
    local cardPos
...
local function stepSellHomeless()
    ...
    -- If still have chips, fallback to cook location
...
local function runFullCycle()
    ...
    -- [9b] WAIT APPROVAL
    if not retryable("WAIT_APPROVAL", stepWaitApproval, 2) then
        setStatus("Approval timeout, continue...", Color3.fromRGB(255, 200, 80))
    end
...
print("[MULTI FARM] Loaded · Combined fully + card scam + chips")