-- ================================================================
-- 🚀 UNIFIED FULL CYCLE — APARTMENT + MARSHMALLOW + CARD + CHIPS
...
-- ================================================================
-- GHOST MODE (untuk blink tembus tembok)
-- ================================================================
...
-- ================================================================
-- BLINK TP (UNDERGROUND — nembus tembok, buat ke dealer)
-- ================================================================
...
-- BypassTP untuk setup apartment (butuh respawn)
...
Title.Text = "🔥 UNIFIED FULL CYCLE v1"
...
local function step_applyCard()
    ...
    return true -- timeout dianggap lolos, akan dicek di claim step
end
...
-- [8] Chips mission A→B→C→D → open pot → mulai cook
...
local function runFullCycle()
    ...
    while ST.running do
        ...
        -- [3] Beli bahan MS
        if not step_buyMarshmallowIngredients() then break end
        if not ST.running then break end

        -- [4] Tuang Water
        if not step_putWater() then
        ...
        -- [7] Beli bahan Chips
        if not step_buyChipsIngredients() then
            updateStatus("Bahan chips gagal", Color3.fromRGB(255, 80, 80))
            task.wait(2); goto continue
        end
        if not ST.running then break end

        -- [8] Chips mission (menggunakan time 60s cook bg)
        if not step_chipsMission() then
        ...
        -- [9] Sugar + Gelatin (47s)
        if not step_sugarGelatin() then break end
        if not ST.running then break end
...
print("[UNIFIED FULL CYCLE] Loaded! Semua step siap dijalankan otomatis.")