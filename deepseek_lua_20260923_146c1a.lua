-- ================================================================
-- MULTI FARM v3 — skip lock apt + loading screen per TP
-- ================================================================
...
local STATE = {
    ...
    targetChips      = 3,
    hasApartment     = false,
    apartmentLocked  = false,   -- v3: skip lock setelah 1x berhasil
    aptKitchen       = nil,
    aptDoor          = nil,
}

-- ================================================================
-- LOADING SCREEN (muncul tiap TP)
-- ================================================================
local UI_Target = (gethui and gethui()) or CoreGui or LP:WaitForChild("PlayerGui")
for _, nm in ipairs({"MultiFarm_UI", "MultiFarm_Loading", "AutoFullCycle_UI", "LENGER_CardScam_UI", "LuzorHub", "LuzorLoadingScreen"}) do
    local o = UI_Target:FindFirstChild(nm)
    if o then pcall(function() o:Destroy() end) end
end

local LoadGui = Instance.new("ScreenGui")
LoadGui.Name = "MultiFarm_Loading"
...
local LoadBox = Instance.new("Frame", LoadFrame)
LoadBox.AnchorPoint = Vector2.new(0.5, 0.5)
LoadBox.Position = UDim2.new(0.5, 0, 0.5, 0)
LoadBox.Size = UDim2.new(0, 220, 0, 70)
LoadBox.BackgroundColor3 = Color3.fromRGB(12, 12, 16)
...
lbStroke.Color = Color3.fromRGB(0, 200, 100)
lbStroke.Thickness = 1.5

local LoadText = Instance.new("TextLabel", LoadBox)
LoadText.Size = UDim2.new(1, -20, 1, -20)
LoadText.Position = UDim2.new(0, 10, 0, 10)
LoadText.BackgroundTransparency = 1
LoadText.Text = "wait..."
LoadText.TextColor3 = Color3.fromRGB(100, 255, 180)
LoadText.Font = Enum.Font.GothamBlack
LoadText.TextSize = 20

local loadDotThread = nil
local function showLoading()
    pcall(function()
        LoadGui.Enabled = true
        LoadText.Text = "wait."
    end)
    if loadDotThread then task.cancel(loadDotThread) end
    loadDotThread = task.spawn(function()
        local n = 0
...
local function hideLoading()
    pcall(function() LoadGui.Enabled = false end)
end
...
local function stepLockApartment()
    -- v3: kalau sudah pernah lock sukses, skip
...
print("[MULTI FARM v3] Loaded · skip lock apt · loading screen per TP")