-- ============================================================
-- AUTO FULL CYCLE v2 — STABIL & SMOOTH
-- Speed 80 stud/s · Hop 8 stud · Anchor-lock saat TP
-- ============================================================
...
-- ============================================================
-- TP SETTINGS (LEBIH LAMBAT & STABIL)
-- ============================================================
local MAX_SPEED = 80      -- ⬅️ turun dari 150 → 80
local HOP_DIST  = 8       -- ⬅️ turun dari 15 → 8 (langkah lebih halus)
local MIN_DELAY = 0.06    -- ⬅️ naik dari 0.04
local MAX_DELAY = 0.6     -- ⬅️ naik dari 0.5
local SETTLE_WAIT = 0.45  -- ⬅️ jeda baru setelah sampai (biar ban settle)

-- ============================================================
-- JEDA AKSI (DILAMATIN)
-- ============================================================
local DELAY = {
    afterTP      = 1.0,   -- ⬅️ 0.6 → 1.0
    insertPotato = 2.8,   -- ⬅️ 2.0 → 2.8
    cookProcess  = 2.8,   -- ⬅️ 2.0 → 2.8
    insertFlour  = 2.8,   -- ⬅️ 2.0 → 2.8
    potOpen      = 1.3,   -- ⬅️ 0.8 → 1.3
    claimChips   = 2.2,   -- ⬅️ 1.5 → 2.2
    panasinWait  = 2.5,   -- ⬅️ 1.8 → 2.5
    sellWait     = 0.6,   -- ⬅️ 0.3 → 0.6
    cookTime     = 60,
}
...
-- ============================================================
-- VEHICLE STABILIZER HELPERS
-- ============================================================
-- Simpan & anchor semua part kendaraan sementara supaya physics
-- nggak ngamuk (ban lepas / bug) saat hop TP
local function anchorVehicle(vehicle)
    local saved = {}
...
local function restoreVehicle(saved)
...
local function hopTP(targetPos)
    ...
    -- 🔒 Anchor semua part kendaraan biar ban nggak rusak
    local savedAnchors = nil
    if vehicle then
        savedAnchors = anchorVehicle(vehicle)
        zeroVehicleVelocity(vehicle)
        task.wait(0.15)  -- beri waktu physics tenang
    end
    ...
    if vehicle and vRoot then
        ...
        task.wait(SETTLE_WAIT)  -- ⬅️ biar ban "duduk" dulu

        -- 🔓 Lepas anchor, kendaraan kembali normal
        if savedAnchors then restoreVehicle(savedAnchors) end
        zeroVehicleVelocity(vehicle)  -- zero sekali lagi setelah unanchor
...
local function farmOneChip()
    ...
    firePromptAt(pot, 8)
    local t0 = os.clock()
    while ST.running and (os.clock() - t0) < 6 do
...
print("[AUTO FULL CYCLE v2 · STABIL] Loaded! Speed 80 · Hop 8 · Anchor-lock")